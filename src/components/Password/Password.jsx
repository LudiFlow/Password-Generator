...
export default function Password(options, passwordLength) {
  let password = [];
    ...
    for (let i = 0; i < passwordLength; i++) {
      ...
      while (isAllowed === false) {
        ...
        if (options.includes(randomNum.toString())) {
          let optionLength = Math.floor(
            Math.random() * (passwordOption[randomNum.toString()].length - 0) +
              0
          );
          password.push(
            passwordOption[randomNum.toString()].slice(
              optionLength,
              optionLength + 1
            )
          );
